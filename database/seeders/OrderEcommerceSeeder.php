...
class OrderEcommerceSeeder extends BaseSeeder
{
                ...
                if ($isMarketplace && $order->store->id && $order->store->customer->id) {
                    ...
                    if ($vendorInfo->id) {
                        ...
                        try {
                            ...
                            Revenue::query()->create(array_merge([
                                'order_id' => $order->id,
                            ], $data));
    ...
    private function calculatorCommissionFeeByProduct($orderProducts): float|int
    {
        ...
        foreach ($orderProducts as $orderProduct) {
            ...
            $commissionSetting = CategoryCommission::query()->whereIn('product_category_id', $listCategories)->orderBy('commission_percentage', 'desc')->first(); // lay commission setting cao nhat
